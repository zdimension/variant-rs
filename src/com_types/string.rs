...
pub struct OwnedComString(pub *mut u16);
...
impl TryFrom<String> for ComString {
    ...
    fn try_from(s: String) -> Result<Self, Self::Error> {
        let u16 = widestring::U16CString::from_str(s)?;
        use winapi::um::oleauto::SysAllocStringLen;
        Ok(ComString(unsafe {
            SysAllocStringLen(u16.as_ptr(), u16.len() as u32)
        }))
    }
}

impl OwnedComString {
    pub fn take_ownership(s: ComString) -> Self {
        OwnedComString(s.0)
    }
}

impl Drop for OwnedComString {
    fn drop(&mut self) {
